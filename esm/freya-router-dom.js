...
import { KeepAlive, Provider } from 'freya-keep-alive';
...
if (process.env.NODE_ENV !== "production") {
  ...
  NormalSwitch.prototype.componentDidUpdate = function (prevProps) {
    process.env.NODE_ENV !== "production" ? warning(!(this.props.location && !prevProps.location), '<NormalSwitch> elements should not change from uncontrolled to controlled (or vice versa). You initially used no "location" prop and then provided one on a subsequent render.') : void 0;
    process.env.NODE_ENV !== "production" ? warning(!(!this.props.location && prevProps.location), '<NormalSwitch> elements should not change from controlled to uncontrolled (or vice versa). You provided a "location" prop initially but omitted it on a subsequent render.') : void 0;
  };
}
...
function renderRoutes(routes) {
  if (lte10) {
    return React.createElement(Provider, null, React.createElement(NormalSwitch, null, routes.map(function (route, idx) {
